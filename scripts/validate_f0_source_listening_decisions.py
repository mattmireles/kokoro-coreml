#!/usr/bin/env python3
"""Validate filled human listening decisions for F0-source candidates."""
...
def _load_rows(path: Path) -> list[dict[str, str]]:
    """Read decision rows and verify the expected CSV surface."""
...
def validate_rows(
    ...
    *,
    ...
    """Validate human decisions for F0-source listening rows."""

    errors: list[str] = []
    decisions = Counter((row.get("human_decision") or "").strip().lower() for row in rows)
    gate_counts = Counter((row.get("waveform_gate_decision") or "").strip().lower() for row in rows)

    for row in rows:
        label = row.get("label") or "<missing label>"
        decision = (row.get("human_decision") or "").strip().lower()
        gate = (row.get("waveform_gate_decision") or "").strip().lower()
        ...
        if decision == "caveat" and not notes:
            errors.append(f"{label}: human_decision=caveat requires notes")
        if gate == "reject_without_listening" and decision in PASSING_DECISIONS:
            errors.append(f"{label}: waveform gate rejected sample but human_decision={decision}")
...
def main() -> int:
    ...
    if args.json:
        ...
        print(
            "f0_source_listening_decisions "
            f"rows={summary['rows']} decisions={summary['decision_counts']} "
