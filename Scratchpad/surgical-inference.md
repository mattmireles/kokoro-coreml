...
The M2 Ultra synthesizes 30 seconds of audio in 379 milliseconds. The M1 Mini — the cheapest Apple Silicon Mac sold — completes the same workload in 1.96 seconds, 14× faster than real-time. Every machine at every duration operates far above real-time, sufficient for streaming TTS with headroom for concurrent workloads. These rows use the exact-duration duration-model path (§4.2); a deployment shipping the library's padded-package default pays more at the duration stage than these tables show.
