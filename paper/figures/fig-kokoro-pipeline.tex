...
\definecolor{padc}{HTML}{DCD6E8}     % host materialization (padding) -- not a named Swift stage; distinct from ioblk
...
\begin{tikzpicture}[
  ...
  padc/.style={stage, fill=padc, densely dotted, text width=52mm, minimum height=7mm},
...
\node[ann, right=4mm of pad.east]   {Host materialization\\(not a named Swift stage)};
...
\node[anchor=north east, draw=edgec, line width=0.6pt, rounded corners=2pt,
  ...
  {\tikz\node[swatch,fill=swiftc]{};}~~Native Swift (CPU)\\[2.5pt]
  {\tikz\node[swatch,fill=padc]{};}~~Host materialization (padding)};
