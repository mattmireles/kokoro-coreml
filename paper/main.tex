...
\begin{figure}[t]
\centering
\IfFileExists{figures/fig-kokoro-pipeline.pdf}{\includegraphics[width=0.8\linewidth]{fig-kokoro-pipeline}}{\pendingfig{fig-kokoro-pipeline}}
...
%% VOLATILE: §5.1 dispersion sentences may receive coordinator patches after conversion.
\subsection{Experimental Setup}\label{sec:cs1-setup}

\textbf{Hardware.} Three Apple Silicon Macs spanning the consumer-to-workstation range, plus two iPhones (\S\ref{sec:cs1-iphone}):
...
\begin{figure}[t]
\centering
\IfFileExists{figures/fig-cs1-latency.pdf}{\includegraphics[width=\linewidth]{fig-cs1-latency}}{\pendingfig{fig-cs1-latency}}
...
Warm median wall time, milliseconds, counterbalanced A/D/E/F run. The Config F build in this ledger predates the June optimizations; its absolute times are higher than \S\ref{sec:cs1-headline} (e.g. 476 ms vs 379 ms for 30 s on M2 Ultra), so the speedups below are lower bounds for the current pipeline. The April canonical input set had four lengths; the 10 s bucket entered the frozen input set with the June bakeoff, so this ledger has no 10 s row.
...
\begin{figure}[t]
\centering
\IfFileExists{figures/fig-mrt2-pipeline.pdf}{\includegraphics[width=\linewidth]{fig-mrt2-pipeline}}{\pendingfig{fig-mrt2-pipeline}}
...
\begin{figure}[t]
\centering
\IfFileExists{figures/fig-ane-ladder.pdf}{\includegraphics[width=\linewidth]{fig-ane-ladder}}{\pendingfig{fig-ane-ladder}}
...
\begin{figure}[t]
\centering
\IfFileExists{figures/fig-power.pdf}{\includegraphics[width=\linewidth]{fig-power}}{\pendingfig{fig-power}}
...
\textbf{Audio quality evaluation is automated, not human --- and it found real deviations.} To move beyond informal listening checks, we ran a structured perceptual evaluation of Case Study 1's shipped pipeline: the PyTorch eager reference (float32, CPU) versus the Swift + Core ML pipeline under its shipped staged compute policy, on the four frozen benchmark inputs (2.8--27.4 s), judged by a multimodal audio model (Gemini) in blind paired lineups with a known-bad control and repeat-vote agreement, alongside the objective waveform gate. The short and medium buckets pass cleanly --- the judge rates the Core ML output ``perceptually indistinguishable'' from the reference (same quality class, no artifacts) while unambiguously rejecting a speech-shaped static control that the objective waveform gate cannot catch. The long buckets do not: the 15-second input fails 3/3 votes on phrase-boundary pauses that the Core ML duration model renders 40--110 ms longer than the reference (heard as ``unnatural silent gaps''), and the 30-second input fails 2/2 votes on a mid-band spectral tilt (1--9 kHz energy at 0.35--0.48$\times$ of the gain-matched reference) heard as ``background static and hiss'' --- deviations invisible to per-stage numerical parity, which compares stages under matched inputs rather than end-to-end renders. This is an automated model-judged evaluation, not a human MOS study: it establishes reproducible perceptual parity for short and medium utterances and documents two concrete, localized quality deviations on long ones, but it does not calibrate absolute quality scores. Commands, verdicts, and signal analyses are recorded in the repository (Appendix~\ref{app:artifacts}). Case Study 2 reports token-level parity, embedding-space adherence and stereo-correlation metrics against the MLX reference, and blind automated listening gates with known-bad controls --- also short of a formal MOS study, and its adherence scores sit below the upstream runtime-CFG reference because the on-device pipeline bakes guidance at export time.
